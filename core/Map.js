(function(pulse) {
    ...
    Map.prototype.addLayer = function(layer, options) {
        if( typeof this.layers[layer] === 'object')
            return;
    ...
    Map.prototype.getElementByZ = function(layer) {
        ...
        var orderedElem = [], elements = this.layers[layer].elements, i;
    ...
    Map.prototype.displayElement = function(elem) {
        ...
        if(elem.rotation != 0 || elem.rotation != 360)
            this.context.rotate(elem.rotation * Math.PI / 180);
    ...
    Map.prototype.userEvent = function(px, py, click) {
        for(layer in this.layers) {
            if(this.layers[layer].options.mouse === false)
                continue;
